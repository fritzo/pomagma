...
struct HashExprPtr {
    size_t operator()(const std::shared_ptr<Expr> &expr) const {
        ...
        std::tuple<Arity, size_t, const Expr *, const Expr *> data{
            expr->arity, hash_data(expr->name.data(), expr->name.size()),
            expr->args[0].get(), expr->args[1].get()};
...
class Reducer {
   public:
    ...
    Term reduce(const std::string &token, const InjectiveFunction *,
                const Term &key) {
        return key ? new_term(INJECTIVE_FUNCTION, token, key) : Term();
    }

    Term reduce(const std::string &token, const BinaryFunction *,
                const Term &lhs, const Term &rhs) {
        return lhs and rhs ? new_term(BINARY_FUNCTION, token, lhs, rhs)
                           : Term();
    }

    Term reduce(const std::string &token, const SymmetricFunction *,
                const Term &lhs, const Term &rhs) {
        return lhs and rhs ? new_term(SYMMETRIC_FUNCTION, token, lhs, rhs)
                           : Term();
    }

    Term reduce(const std::string &token, const UnaryRelation *,
                const Term &key) {
        return key ? new_term(UNARY_RELATION, token, key) : Term();
    }

    Term reduce(const std::string &token, const BinaryRelation *,
                const Term &lhs, const Term &rhs) {
        return lhs and rhs ? new_term(BINARY_RELATION, token, lhs, rhs)
                           : Term();
    }
    ...
    Term reduce_hole() { return new_term(HOLE, ""); }

    Term reduce_var(const std::string &name) { return new_term(VAR, name); }

    Term reduce_error(const std::string &) { return Term(); }

   private:
    Term new_term(Arity arity, const std::string &name, Term arg0 = Term(),
                  Term arg1 = Term()) {
...
class Parser : public ExprParser<Reducer> {
   public:
    Parser(Signature &signature, ExprSet &deduped,
...
Theory parse_theory(Signature &signature,
...
inline void propagate_constraint(
    const Expr *expr, const std::unordered_map<const Expr *, State> &states,
    ...
    switch (expr->arity) {
        ...
        case SYMMETRIC_FUNCTION: {
            ...
            message_queues[val].push_back(
                approximator.lazy_binary_function_lhs_rhs(name, states.at(lhs),
                                                          states.at(rhs)));
            message_queues[rhs].push_back(
                approximator.lazy_binary_function_lhs_val(name, states.at(lhs),
                                                          states.at(val)));
            message_queues[lhs].push_back(
                approximator.lazy_binary_function_rhs_val(name, states.at(rhs),
                                                          states.at(val)));
        } break;
        ...
        case HOLE:
            break;  // no information
        case VAR:
            break;  // no information
    }
}

inline void log_change_count(std::unordered_map<const Expr *, State> &states,
                             size_t change_count) {
    if (states.size() > 10) {
...
inline size_t propagate_step(
    ...
    std::unordered_map<const Expr *, std::vector<State>> &message_queues,
    const Theory &theory, Approximator &approximator) {
    ...
    for (auto &i : states) {
        ...
        POMAGMA_ASSERT1(approximator.expensive_refines(updated_state, state),
...
Trool lazy_validate(const Theory &theory, Approximator &approximator) {
    ...
    while (propagate_step(states, message_queues, theory, approximator)) {
        ++step;
        POMAGMA_ASSERT(step < max_steps, "propagation failed to converge in "
                                             << max_steps << " steps");
    }
