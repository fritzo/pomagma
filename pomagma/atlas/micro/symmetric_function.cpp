...
SymmetricFunction::SymmetricFunction(
    ...
    : m_lines(carrier),
      ...
      m_tiles(pomagma::alloc_blocks<Tile>(unordered_pair_count(m_tile_dim))),
            ...
            for (size_t _i = 0; _i < ITEMS_PER_TILE; ++_i)
                for (size_t _j = 0; _j < ITEMS_PER_TILE; ++_j) {
                    ...
                    if (not(support().contains(i) and support().contains(j))) {
                        POMAGMA_ASSERT(not val, "found unsupported value "
                                                    << i << ',' << j << ','
                                                    << val);
                    } else if (val) {
                        POMAGMA_ASSERT(defined(i, j), "found undefined value "
                                                          << i << ',' << j
                                                          << ',' << val);
                    } else {
                        POMAGMA_ASSERT(
