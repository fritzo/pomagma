...
inline float Router::get_prob(const Segment& segment,
...
inline void Router::add_weight(float weight, const Segment& segment,
...
void Router::fit_language(
    ...
    // EM algorithm to optimize PCFG production rule probabilities from training
    // corpus
    const size_t item_count = m_carrier.item_count();
    ...
    // EM iterations: alternate between E-step (compute expectations) and M-step
    // (maximize parameters)
    bool changed = true;
    while (changed) {
        changed = false;
        ...
        // Redistribute observed counts proportionally to rule usage
        // probabilities
        update_weights(ob_probs, symbol_counts, ob_counts, symbol_weights,
                       ob_weights, reltol);

        // M-step: normalize symbol weights to get new production rule
        // probabilities
        ...
        for (size_t i = 0; i < m_types.size(); ++i) {
            ...
            float new_prob =
                symbol_weights[i] / total_weight;  // MLE normalization
            float old_prob = type.prob;
            type.prob = new_prob;
            m_language[type.name] = new_prob;

            // Check convergence: continue if any parameter changed
            // significantly
