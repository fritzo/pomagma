...
static const char *g_log_level_name[4] = {"ERROR   ", "WARNING ", "INFO    ",
                                          "DEBUG   "};
...
#define POMAGMA_WARN(message)             \
    {                                     \
        if (pomagma::Log::level() >= 1) { \
            pomagma::Log(1) << message;   \
        }                                 \
    }
#define POMAGMA_INFO(message)             \
    {                                     \
        if (pomagma::Log::level() >= 2) { \
            pomagma::Log(2) << message;   \
        }                                 \
    }
#define POMAGMA_DEBUG(message)            \
    {                                     \
        if (pomagma::Log::level() >= 3) { \
            pomagma::Log(3) << message;   \
        }                                 \
    }

#define POMAGMA_PRINT(variable) POMAGMA_INFO(#variable " = " << (variable))

#define POMAGMA_ERROR(message)                                                \
    {                                                                         \
        pomagma::Log(0) << message << "\n\t" << __FILE__ << " : " << __LINE__ \
                        << "\n\t" << __PRETTY_FUNCTION__ << "\n";             \
        abort();                                                              \
    }
...
#define POMAGMA_ASSERT(cond, mess)         \
    {                                      \
        if (not(cond)) POMAGMA_ERROR(mess) \
    }
...
#define POMAGMA_ASSERT_EQ(x, y)                                     \
    POMAGMA_ASSERT((x) == (y), "expected " #x " == " #y "; actual " \
                                   << (x) << " vs " << (y))
...
#define POMAGMA_ASSERT_OK POMAGMA_ASSERT5(ok(), "tried to use done iterator")
...
static constexpr size_t WORD_POS_MASK = BITS_PER_WORD - 1;
static constexpr size_t WORD_POS_SHIFT = static_log2i<BITS_PER_WORD>::val();
static_assert(WORD_POS_SHIFT > 0, "bad math");
static constexpr Word FULL_WORD = ~Word(0);
static_assert(FULL_WORD + Word(1) == 0, "FULL_WORD is bad");

static constexpr size_t BYTES_PER_CACHE_LINE = 64;
static constexpr size_t BITS_PER_CACHE_LINE = 512;
