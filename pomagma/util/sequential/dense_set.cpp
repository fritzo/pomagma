...
template void DenseSet::copy_from<uint16_t>(const DenseSet &, const uint16_t *);
template void DenseSet::copy_from<uint32_t>(const DenseSet &, const uint32_t *);
...
void DenseSet::zero() { bzero(m_words, sizeof(Word) * m_word_dim); }
...
void DenseSet::set_ppn(const DenseSet &pos1, const DenseSet &pos2,
                       const DenseSet &neg) {
...
void DenseSet::set_ppnn(const DenseSet &pos1, const DenseSet &pos2,
                        const DenseSet &neg1, const DenseSet &neg2) {
    POMAGMA_ASSERT1(item_dim() == pos1.item_dim(), "pos1.item_dim mismatch");
    POMAGMA_ASSERT1(item_dim() == pos2.item_dim(), "pos2.item_dim mismatch");
...
void DenseSet::set_pnn(const DenseSet &pos, const DenseSet &neg1,
                       const DenseSet &neg2) {
    POMAGMA_ASSERT1(item_dim() == pos.item_dim(), "pos.item_dim mismatch");
    POMAGMA_ASSERT1(item_dim() == neg1.item_dim(), "neg1.item_dim mismatch");
    POMAGMA_ASSERT1(item_dim() == neg2.item_dim(), "neg2.item_dim mismatch");
