...
public:
    ...
    // full table
    const Word * Lx () const { return m_Lx_lines; }
    const Word * Rx () const { return m_Rx_lines; }
    Word * Lx () { return m_Lx_lines; }
    Word * Rx () { return m_Rx_lines; }

    // single line
    const Word * Lx (Ob lhs) const
    {
        POMAGMA_ASSERT_RANGE_(5, lhs, item_dim());
        return m_Lx_lines + (lhs * m_round_word_dim);
    }
    const Word * Rx (Ob rhs) const
    {
    ...
    {
        return DenseSet(item_dim(), const_cast<Word *>(Lx(lhs)));
    }
    DenseSet Rx_set (Ob rhs) const
    {
        return DenseSet(item_dim(), const_cast<Word *>(Rx(rhs)));
    }
};
