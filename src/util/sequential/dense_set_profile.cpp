...
int main ()
{
    ...
    POMAGMA_INFO(
        std::setw(10) << "log2(size)" <<
        std::setw(10) << "iter(kHz)" <<
        std::setw(15) << "iter_insn(kHz)" <<
        std::setw(20) << "likely_disjoint(kHz)" <<
    ...
    for (size_t exponent = min_exponent; exponent < max_exponent; ++exponent) {
        ...
        float freq3 = test_likely_disjoint(exponent, 0.5) / 1000;
        float freq4 = test_unlikely_disjoint(exponent, 0.5) / 1000;
