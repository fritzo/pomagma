...
# These are needed to pacify pyflakes.
goto = None
label = None
...
def _find_labels_and_gotos(code):
    ...
    while True:
        try:
            op1, arg1, op2, arg2, op3 = _STRUCT_ATTR_LOOKUP.unpack_from(
                code.co_code, pos)
        except struct.error:
            break
...
def _patch_code(code):
    ...
    for goto_pos, arg, goto_stack in gotos:
        ...
        if depth_delta > max_depth_delta:
            raise SyntaxError(
                'Jumps out of more than %d nested blocks are not allowed' %
                max_depth_delta)
        ...
        _STRUCT_OP_WITH_ARG.pack_into(
            buf, jump_pos, dis.opmap['JUMP_ABSOLUTE'], target)
