...
class Hasher {
   public:
    ...
    Hasher() : m_state(ADDING), m_data(SHA_DIGEST_LENGTH, 0) {
        POMAGMA_ASSERT(SHA1_Init(&m_context), "SHA1_Init failed");
    }
    ~Hasher() { POMAGMA_ASSERT1(m_state == FINISHED, "Hasher was never used"); }
    ...
    const Digest& finish() {
        ...
        POMAGMA_ASSERT(SHA1_Final(&m_data[0], &m_context), "SHA1_Final failed");
