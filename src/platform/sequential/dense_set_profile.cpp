...
double test_disjoint (size_t exponent, float density, size_t iters = 10000)
{
    size_t item_count = (1 << exponent) - 1;
    DenseSet set1(item_count);
    DenseSet set2(item_count);
    ...
    set2 -= set1;  // .disjoint is usually called with actually disjoint sets

    size_t count;
    Timer timer;
    for (size_t i = 0; i < iters; ++i) {
        count += set1.disjoint(set2);
    }
    return iters / timer.elapsed();
}
...
int main ()
{
    ...
    for (size_t exponent = min_exponent; exponent < max_exponent; ++exponent) {
        ...
        float freq3 = test_disjoint(exponent, 0.5) / 1000;
