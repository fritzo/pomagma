...
inline BinaryRelation& Structure::binary_relation(const std::string& name) {
    auto* result = m_signature.binary_relation(name);
    POMAGMA_ASSERT(result, "missing binary relation " << name);
    return *result;
}

inline NullaryFunction& Structure::nullary_function(const std::string& name) {
...
inline BinaryFunction& Structure::binary_function(const std::string& name) {
