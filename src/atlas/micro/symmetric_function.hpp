...
class SymmetricFunction : noncopyable {
   ...
   public:
    SymmetricFunction(const Carrier &carrier,
                      void (*insert_callback)(const SymmetricFunction *, Ob,
                                              Ob) = nullptr);
    ~SymmetricFunction();
    void validate() const;
...
inline VLr_Table::Iterator SymmetricFunction::iter_val_lhs(Ob val,
                                                           Ob lhs) const {
    POMAGMA_ASSERT5(support().contains(val), "unsupported val: " << val);
    POMAGMA_ASSERT5(support().contains(lhs), "unsupported lhs: " << lhs);
...
inline VLr_Table::Iterator SymmetricFunction::iter_val_rhs(Ob val,
                                                           Ob rhs) const {
