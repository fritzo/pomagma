...
void Sampler::validate() const {
    ...
    float total =
        m_nullary_prob + m_injective_prob + m_binary_prob + m_symmetric_prob;
...
template <class Key>
inline Key sample(const std::unordered_map<Key, float> &probs, float total,
                  rng_t &rng) {
...
// base case
Sampler::BoundedSampler::BoundedSampler(const Sampler &sampler)
    : injective(0),
      binary(0),
      symmetric(0),
...
Sampler::BoundedSampler::BoundedSampler(const Sampler &sampler,
    ...
    : injective(sampler.m_injective_prob * prev.total),
      ...
      compound_total(compound_injective + compound_binary +
                     compound_symmetric) {}
...
static const char *g_sampler_arity_names[]
    __attribute__((unused)) = {"NULLARY", "INJECTIVE", "BINARY", "SYMMETRIC"};

inline Ob Sampler::insert_random_compound(Ob ob, size_t max_depth, rng_t &rng,
                                          Policy &policy) const {
...
inline Ob Sampler::insert_random(size_t max_depth, rng_t &rng,
                                 Policy &policy) const {
    const BoundedSampler &sampler = bounded_sampler(max_depth);
...
inline Ob Sampler::insert_random_nullary(rng_t &rng, Policy &policy) const {
    auto &fun = *sample(m_nullary_probs, m_nullary_prob, rng);
    return policy.sample(fun);
}

inline Ob Sampler::insert_random_injective(Ob key, rng_t &rng,
                                           Policy &policy) const {
    auto &fun = *sample(m_injective_probs, m_injective_prob, rng);
    return policy.sample(fun, key);
}

inline Ob Sampler::insert_random_binary(Ob lhs, Ob rhs, rng_t &rng,
                                        Policy &policy) const {
    auto &fun = *sample(m_binary_probs, m_binary_prob, rng);
    return policy.sample(fun, lhs, rhs);
}

inline Ob Sampler::insert_random_symmetric(Ob lhs, Ob rhs, rng_t &rng,
                                           Policy &policy) const {
