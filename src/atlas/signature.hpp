...
class Signature : noncopyable {
   ...
   public:
    ...
    const std::unordered_map<std::string, UnaryRelation *> &unary_relations()
        const;
...
inline void Signature::declare(Carrier &carrier) {
    POMAGMA_ASSERT(m_carrier == nullptr, "Declared carrier twice");
    m_carrier = &carrier;
}

inline void Signature::declare(const std::string &name, UnaryRelation &rel) {
    m_unary_relations.insert(std::make_pair(name, &rel));
}

inline void Signature::declare(const std::string &name, BinaryRelation &rel) {
    m_binary_relations.insert(std::make_pair(name, &rel));
}

inline void Signature::declare(const std::string &name, NullaryFunction &fun) {
    m_nullary_functions.insert(std::make_pair(name, &fun));
}

inline void Signature::declare(const std::string &name,
                               InjectiveFunction &fun) {
    m_injective_functions.insert(std::make_pair(name, &fun));
}

inline void Signature::declare(const std::string &name, BinaryFunction &fun) {
...
template <class T>
inline T *replace(std::unordered_map<std::string, T *> &map,
                  const std::string &name, T *new_value) {
...
inline Carrier *Signature::replace(Carrier &carrier) {
    return detail::replace(m_carrier, &carrier);
}

inline UnaryRelation *Signature::replace(const std::string &name,
                                         UnaryRelation &rel) {
    return detail::replace(m_unary_relations, name, &rel);
}

inline BinaryRelation *Signature::replace(const std::string &name,
                                          BinaryRelation &rel) {
    return detail::replace(m_binary_relations, name, &rel);
}

inline NullaryFunction *Signature::replace(const std::string &name,
                                           NullaryFunction &fun) {
    return detail::replace(m_nullary_functions, name, &fun);
}

inline InjectiveFunction *Signature::replace(const std::string &name,
                                             InjectiveFunction &fun) {
    return detail::replace(m_injective_functions, name, &fun);
}

inline BinaryFunction *Signature::replace(const std::string &name,
                                          BinaryFunction &fun) {
    return detail::replace(m_binary_functions, name, &fun);
}

inline SymmetricFunction *Signature::replace(const std::string &name,
                                             SymmetricFunction &fun) {
    return detail::replace(m_symmetric_functions, name, &fun);
}

inline UnaryRelation *Signature::unary_relation(const std::string &name) const {
