...
def compile(*infiles, **kwargs):
    ...
    # if up_to_date(infiles, [cpp_out, facts_out, programs_out]):
    #     return
    ...
    symbols = cpp.write_symbols(rules, facts)
    with open(symbols_out, 'w') as f:
        f.write('# This file was auto generated by pomagma using:\n')
        f.write('# python -m pomagma.compiler compile {0}'.format(argstring))
    ...
    programs = cpp.write_programs(rules)
...
def batch_compile():
    ...
    for name, spec in theories.iteritems():
        ...
        if not up_to_date(infiles + [theories_json], [cpp_out, facts_out]):
            params.append({
                'args': infiles,
                'kwargs': {
                    'cpp_out': cpp_out,
