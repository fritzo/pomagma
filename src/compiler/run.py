...
def compile(*infiles, **kwargs):
    ...
    Optional keyword arguments:
        cpp_out=$POMAGMA_ROOT/src/surveyor/<STEM>.laws.cpp
        laws_out=$POMAGMA_ROOT/src/laws/<STEM>.compiled
        laws_out=FILENAME
        extensional=true
    '''
    ...
    cpp_out = kwargs.get(
            'cpp_out',
            os.path.join(POMAGMA_SRC, 'surveyor', '{0}.laws.cpp'.format(stem)))
    laws_out = kwargs.get(
            'laws_out',
            os.path.join(POMAGMA_SRC, 'laws', '{0}.compiled'.format(stem)))
    ...
    cpp.write_laws(code, rules, facts)

    with open(cpp_out, 'w') as f:
        f.write(str(code))

    with open(laws_out, 'w') as f:
