...
Problem formulate (std::shared_ptr<Corpus> corpus)
{
    ...
    for (const auto & ptr : corpus.get()->lines) {
    ...
    intervals::Approximator & approximator)
{
    ...
    switch (constraint->arity) {
        ...
        case SYMMETRIC_FUNCTION: {
            const Term * lhs = constraint->args[0].get();
            const Term * rhs = constraint->args[1].get();
            const Term * val = constraint;
        ...
        case BINARY_RELATION: {
            ...
            if (name == "LESS") {
                ...
                message_queues[val].push_back(
                    approximator.lazy_less_lhs_rhs(
                        map_find(states, lhs),
                        map_find(states, rhs)));
            } else if (name == "NLESS") {
                message_queues[lhs].push_back(
                ...
                message_queues[val].push_back(
                    approximator.lazy_nless_lhs_rhs(
                        map_find(states, lhs),
                        map_find(states, rhs)));
            } else {
...
Solution solve (const Problem & problem, intervals::Approximator & approximator)
{
    ...
    return {problem.corpus, std::move(states)};
}
