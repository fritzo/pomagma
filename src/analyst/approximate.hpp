...
struct Approximation {
    ...
    bool operator==(const Approximation &other) const {
        return ob == other.ob and upper == other.upper and lower == other.lower;
    }
    bool operator!=(const Approximation &other) const {
...
class Approximator : noncopyable {
   public:
    ...
    Approximation find(const NullaryFunction &fun);
    Approximation find(const InjectiveFunction &fun, const Approximation &key);
    Approximation find(const BinaryFunction &fun, const Approximation &lhs,
                       const Approximation &rhs);
    Approximation find(const SymmetricFunction &fun, const Approximation &lhs,
                       const Approximation &rhs);
    Approximation find(const UnaryRelation &pos, const UnaryRelation &neg,
                       const Approximation &arg);
    Approximation find(const BinaryRelation &pos, const BinaryRelation &neg,
                       const Approximation &lhs, const Approximation &rhs);

    Approximation find(const std::string &name);
   ...
   private:
    ...
    bool try_close(Approximation &approx, DenseSet &temp_set);
    void close(Approximation &approx, DenseSet &temp_set);

    size_t test_less();
    template <class Function>
    size_t test_function(const std::string &name, const Function &fun);

    void map(const InjectiveFunction &fun, const DenseSet &key_set,
             DenseSet &val_set, DenseSet &temp_set);
    void map(const BinaryFunction &fun, const DenseSet &lhs_set,
             const DenseSet &rhs_set, DenseSet &val_set, DenseSet &temp_set);
    void map(const SymmetricFunction &fun, const DenseSet &lhs_set,
             const DenseSet &rhs_set, DenseSet &val_set, DenseSet &temp_set);
...
class ApproximateReducer : noncopyable {
   public:
    ...
    Approximation reduce(const std::string &, const NullaryFunction *fun) {
        return m_approximator.find(*fun);
    }

    Approximation reduce(const std::string &, const InjectiveFunction *fun,
                         const Approximation &key) {
        return m_approximator.find(*fun, key);
    }

    Approximation reduce(const std::string &, const BinaryFunction *fun,
                         const Approximation &lhs, const Approximation &rhs) {
        return m_approximator.find(*fun, lhs, rhs);
    }

    Approximation reduce(const std::string &, const SymmetricFunction *fun,
                         const Approximation &lhs, const Approximation &rhs) {
        return m_approximator.find(*fun, lhs, rhs);
    }
