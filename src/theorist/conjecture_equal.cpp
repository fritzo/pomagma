...
std::vector<std::pair<Ob, Ob>> conjecture_equal(
    Structure& structure, const std::vector<float>& probs,
    const std::vector<std::string>& routes, const char* conjectures_file,
    size_t max_count) {
    ...
    for (auto iter = carrier.iter(); iter.ok(); iter.next()) {
        ...
        for (auto iter = carrier.iter(); iter.ok(); iter.next()) {
            ...
            if (NLESS.find_Lx(lhs, rhs)) {
                continue;
            }
            if (NLESS.find_Rx(rhs, lhs)) {
                continue;
            }
    ...
    if (conjectures.size() > max_count) {
        std::nth_element(conjectures.begin(), conjectures.begin() + max_count,
                         conjectures.end(), sort_by_prob);
...
size_t conjecture_equal(Structure& structure, const char* language_file,
                        const char* conjectures_file, size_t max_count) {
    ...
    return conjecture_equal(structure, probs, routes, conjectures_file,
                            max_count);
}
...
size_t conjecture_equal(Structure& structure, const std::vector<float>& probs,
                        const std::vector<std::string>& routes,
                        const char* conjectures_file, size_t max_count) {
    auto conjectures = detail::conjecture_equal(structure, probs, routes,
                                                conjectures_file, max_count);
...
void try_prove_nless(Structure& structure, const char* language_file,
                     ...
                     const char* theorems_file, size_t max_count) {
    std::vector<float> probs;
    std::vector<std::string> routes;
    ...
    const auto conjectures = detail::conjecture_equal(
        structure, probs, routes, conjectures_in_file, max_count);

    detail::try_prove_nless(structure, language, routes, conjectures,
